...
<head>
    ...
    <script type="text/javascript">
            ...
            Confirm.show('溫馨提示', '您確定要刪除' + name + '嗎？', {
                'Delete': {
                    ...
                    'callback': function () {
                        //此处需要调用ajax
                        var params = {"id": id};
                        $.post("${pageContext.request.contextPath}/speaker/speakerDel", params, function (data) {
                            if (data == 'success') {
        ...
        function showName(obj, id, type) {
            var name = $(obj).text();
            if (type == 1) {
                $("#speakerName1").html(name + "<span class='caret'></span>");
                $("#speakerId").val(id);
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse"
            ...
            <p class="navbar-text navbar-right">
                <span>${sessionScope.USERNAME}</span> <i class="glyphicon glyphicon-log-in" aria-hidden="true"></i>&nbsp;&nbsp;
...
<div class="container">
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <!-- 查询相关组件 -->
            <form class="navbar-form navbar-right" action="${pageContext.request.contextPath}/speaker/list"
                  method="post">
                <div class="btn-group">
                    <button type="button" id="speakerName1"
                    ...
                    <input type="hidden" name="speakerName" id="speakerName" value=""/>
                </div>
...
<div class="container" style="margin-top: 20px;">
    ...
    -->
    <form id="form2" action="delBatchSpeakers" method="post">
        <table class="table table-bordered table-hover"
               style="text-align: center;table-layout:fixed">
            <thead>
            <tr class="active">
                <th style="width:3%"><input type="checkbox" onclick="selectAll(this)" id="checkAllId"/></th>
            ...
            <c:forEach items="${page.rows}" var="speaker" varStatus="status">
                <tr>
                    <td>
                        <input type="checkbox" name="ids" value="${speaker.id}" onclick="selectOne(this)"/>
                    </td>
                    <td>${status.count}</td>
                    <td>${speaker.speakerName}</td>
                    <td style="overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">${speaker.speakerDesc}</td>
                    <td>${speaker.speakerJob}</td>
                    <td><img src="${speaker.headImgUrl}" style="height: 20px; width: 20px"/></td>
                    ...
                    <td>
                        <a onclick="return delSpeakerById(this,'${speaker.id}','${speaker.speakerName}')">
